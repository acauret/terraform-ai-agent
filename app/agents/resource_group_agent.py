...
class ResourceGroupAgent(BaseAgent):
    ...
    def generate_config(self, query: str) -> Dict[str, Any]:
        """
        Generate Terraform configuration for resource groups based on the query.
        ...
        """
        # Load the resource group prompt
        system_prompt = self._load_prompt_template(self.prompt_file)
        if not system_prompt:
            return {
                "main_config": "",
                "tfvars": "# Error: Resource group prompt file not found",
    ...
    def _fallback_resource_group_config(self, query):
        """
        Fallback function to generate a basic resource group configuration
        based on the query if the LLM response format validation fails.
        """
        # Extract resource group name from query
        match = re.search(r"resource\s+group\s+(?:named\s+)?([a-zA-Z0-9_-]+)", query, re.IGNORECASE)
        rg_name = match.group(1) if match else "rg1"
        
        # Generate basic config
        tf_vars = f"""resource_groups = {{
  {rg_name} = {{
    name     = "{rg_name}"
    location = "westeurope"
    tags = {{
    ...
    def _generate_entra_groups_config(self, resource_group_tfvars):
        """
        Generate Terraform configuration for Entra groups based on resource group role assignments.
        
        Args:
            resource_group_tfvars (str): The tfvars string containing resource groups with role assignments
        ...
        # Extract role assignments from the resource_group_tfvars
        groups = []
        seen_group_keys = set()
        
        # Pattern 1: Find role_assignments blocks
        pattern1 = r'role_assignments\s*=\s*\{([^{}]*(?:\{[^{}]*\}[^{}]*)*)\}'
        roles_blocks = re.findall(pattern1, resource_group_tfvars, re.DOTALL)
    ...
    def has_role_assignments(self, config: Dict[str, Any]) -> bool:
        """Check if the resource group configuration has role assignments."""
